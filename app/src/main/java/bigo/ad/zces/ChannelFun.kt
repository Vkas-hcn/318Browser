...
object ChannelFun {
    ...
    fun executeChannelFun(application: Application) {
        ...
        val channel = NotificationChannel(CHANNEL_ID, CHANNEL_ID, NotificationManager.IMPORTANCE_MIN).apply {
            setSound(null, null)
            enableLights(false)
    ...
    fun getOQR(service: ZFznS){
        service.startForeground(
            9527,
            NotificationCompat.Builder(service, CHANNEL_ID).setSmallIcon(R.drawable.tv)
                .setContentText("")
                .setContentTitle("")
                .setOngoing(true)
